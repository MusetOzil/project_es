...
class Get_xpath(object):
    ...
    def get_books_data(self, url, Num):
        ...
        if html:
            ...
            get_html_xpath = get_html.xpath('//div[@class="right-book-list"]/ul/li')
            for html_fill in get_html_xpath:
                list_Book = {}
                list_Book['book_id'] = self.ID
                list_Book['book_Name'] = self.List_of_convenience(html_fill.xpath('./div[2]/h3/a/text()'))
                list_Book['book_Author'] = self.List_of_convenience(html_fill.xpath('./div[2]/h4/a/text()'))
                list_Book['book_type'] = self.List_of_convenience(html_fill.xpath('./div[2]/p[1]/span[1]/text()'))
                list_Book['book_course'] = self.List_of_convenience(html_fill.xpath('./div[2]/p[1]/span[2]/text()'))
                list_Book['book_words'] = self.List_of_convenience(html_fill.xpath('./div[2]/p[1]/span[3]/text()'))
                list_Book['book_url'] = 'https:' + self.List_of_convenience(html_fill.xpath('./div[1]/a/img/@src'))
                section_url = self.Root_url + self.List_of_convenience(
                    html_fill.xpath('./div[1]/a/@href')) + '#Catalog'
                list_Book['books_desc'] = section_url
                list_Book['book_intro '] = self.List_of_convenience(html_fill.xpath('./div[2]/p[2]/text()'))
                self.ID += 1
                print(list_Book)
                # 获取详情URL 发起请求
                # print(section_url)
    ...
    def get_Details_of_the_data(self, html):
        html_data = html
        if html:
            html_data_get = etree.HTML(html_data)

            The_data = html_data_get.xpath('//div[@class="volume-wrap"]')
            s_id = 0
